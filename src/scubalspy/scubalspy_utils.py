...
class FileUtils:
    ...
    @staticmethod
    def read_file(logger: ScubalspyLogger, file_path: str) -> str:
        """
        Reads the file at the given path and returns the contents as a string.
        """
        encodings = ["utf-8-sig", "windows-1252", "utf-16"]
        try:
            ...
            raise ScubalspyException("File read failed.") from None
        logger.log(f"File read '{file_path}' failed: Unsupported encoding.", logging.ERROR)
        raise ScubalspyException(f"File read '{file_path}' failed: Unsupported encoding.") from None
    
    @staticmethod
    def download_file(logger: ScubalspyLogger, url: str, target_path: str) -> None:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise ScubalspyException("Error downoading file.")
            with open(target_path, "wb") as f:
                shutil.copyfileobj(response.raw, f)
        except Exception as exc:
            logger.log(f"Error downloading file '{url}': {exc}", logging.ERROR)
            raise ScubalspyException("Error downoading file.") from None

    @staticmethod
    def download_and_extract_archive(logger: ScubalspyLogger, url: str, target_path: str, archive_type: str) -> None:
        """
        ...
        try:
            ...
            if archive_type in ["zip", "tar", "gztar", "bztar", "xztar"]:
                ...
                raise ScubalspyException(f"Unknown archive type '{archive_type}'")
        except Exception as exc:
            logger.log(f"Error extracting archive '{tmp_file_name}' obtained from '{url}': {exc}", logging.ERROR)
            raise ScubalspyException("Error extracting archive.") from exc
        finally:
...
class DotnetVersion(str, Enum):
    """
    scubalspy supported dotnet versions
    """
    V4 = "4"
    V6 = "6"
    V7 = "7"
...
class PlatformUtils:
    ...
    def get_platform_id() -> PlatformId:
        ...
        if system in system_map and machine in machine_map:
            ...
            raise ScubalspyException("Unknown platform: " + system + " " + machine + " " + bitness)
        ...
        try:
            ...
            for line in result.stdout.decode('utf-8').split('\n'):
                if line.startswith('Microsoft.NETCore.App'):
                    ...
                    break
            if version == '':
                raise ScubalspyException("dotnet not found on the system")
            if version.startswith("8"):
                ...
                raise ScubalspyException("Unknown dotnet version: " + version)
        except (FileNotFoundError, subprocess.CalledProcessError):
            try:
                result = subprocess.run(["mono", "--version"], capture_output=True, check=True)
                return DotnetVersion.VMONO
            except (FileNotFoundError, subprocess.CalledProcessError):
                raise ScubalspyException("dotnet or mono not found on the system")
